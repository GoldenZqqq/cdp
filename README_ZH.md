...
稳定状态码为 `clean`、`changed`、`path_missing`、`not_git`、`scan_timeout`、`scan_failed`；稳定关注原因为 `dirty`、`untracked`、`behind`、`path_missing`、`scan_timeout`、`scan_failed`。消费者应拒绝不支持的 schema 主版本，但可以忽略后续新增字段。
